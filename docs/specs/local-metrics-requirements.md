...
| 单位 | 1/5/15 分钟系统负载数值；正常窗口的 stdout 描述为“1 分钟系统负载：值（1分钟，CPU核数=N，负载/核数<=比值）” |
...
| 脱敏 | 无 |
| 文档基线 | 每个负载窗口 ≤ 核数 → OK，并显示“窗口系统负载：值（窗口，CPU核数=N，负载/核数<=比值）”；持续 > 核数 → 等级缺失 → UNKNOWN（建议外部配置：如持续 > 核数 → WARN）。metric 级兼容状态仍以 load_1m 为准。 |
