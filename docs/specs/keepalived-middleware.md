...
| --- | --- | --- | --- |
| `local.keepalived.process.present` | Keepalived 进程存在性 | `ps -eo pid=,comm=,args= \| grep -E '^[[:space:]]*[0-9]+[[:space:]]+keepalived[[:space:]]'`；`comm=keepalived` 才算真实进程，命令回显不算运行；白名单主机未运行为 CRIT，非白名单主机跳过模块 | P0「Keepalived本节点服务」 |
...
1. 先用 `ps -eo pid=,comm=,args=` 筛选 `comm=keepalived` 的真实进程；从进程命令行提取二进制路径和 `-f`
   指定的配置路径。
