...
- `local.cpu.load_1m`：从同一份 `/proc/loadavg` 事实读取 1 分钟、5 分钟、15 分钟系统负载，并与 CPU 核数比较；stdout 逐行显示“1 分钟系统负载：值（1分钟，CPU核数=N，负载/核数<=比值）”，括号内容来自 JSON `evidence.details`；metric 级兼容值仍为 1 分钟负载；
...
中文字段名、状态、规范化值和单位，例如 `CPU 使用率: 12.34 %` 或 `1 分钟系统负载：0.52（1分钟，CPU核数=8，负载/核数<=0.07）`。失败或未选择的
