...
def _cpu_load_detail_lines(metric: Mapping[str, Any]) -> Optional[List[tuple[str, str]]]:
    """Return load-average window lines from JSON evidence only.

    The renderer never re-evaluates thresholds. New facts carry the judgement
    text generated by normalization; old facts without details fall back to
    the ordinary metric line for compatibility.
    """
    details = (metric.get("evidence") or {}).get("details")
    if not isinstance(details, list):
        return None
    ...
    *,
    ...
    for metric in doc.get("metrics", []):
        ...
        if metric.get("metric_id") == "local.cpu.load_1m":
            detail_lines = _cpu_load_detail_lines(metric)
        elif metric.get("metric_id") in {
