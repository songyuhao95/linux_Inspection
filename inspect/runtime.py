...
def _bundle_files(root: Path) -> list[Path]:
    ...
    return sorted(
        ...
        # Sort the canonical POSIX relative path rather than Path objects.
        # Path ordering differs between Windows and Linux (notably for ASCII
        # case), so sorting Path instances would produce different bundle
        # hashes for the same files on different control hosts.
