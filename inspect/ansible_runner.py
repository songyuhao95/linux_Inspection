...
_ADDITIONAL_COMMANDS = {
    ...
    "local.mysql.replication.lag": "replica_status=$(\"$mysql_bin\" --defaults-file=<MYSQL_CONF> --socket=<MYSQL_SOCKET> --protocol=SOCKET --connect-timeout=3 --batch -u<USER> -e \"SHOW REPLICA STATUS\\G\" 2>&1); replica_rc=$?; if [ \"$replica_rc\" -ne 0 ]; then printf '%s\\n' \"$replica_status\"; exit \"$replica_rc\"; elif [ -z \"$replica_status\" ]; then printf '%s\\n' MYSQL_REPLICA_NOT_CONFIGURED=true; else printf '%s\\n' \"$replica_status\" | egrep 'Seconds_Behind_Source|Read_Source_Log_Pos|Exec_Source_Log_Pos|Retrieved_Gtid_Set|Executed_Gtid_Set'; fi",
    ...
    "local.rabbitmq.alarm.partition": "alarms=$(<RABBITMQ_DIAGNOSTICS> check_local_alarms 2>&1); alarm_rc=$?; cluster=$(<RABBITMQCTL> cluster_status 2>&1); cluster_rc=$?; if [ \"$alarm_rc\" -eq 0 ] && [ \"$cluster_rc\" -eq 0 ] && ! printf '%s\\n' \"$alarms\" \"$cluster\" | grep -Eqi 'memory alarm|disk alarm|partition[^[:space:]]*[[:space:]]*[:=][[:space:]]*\\[[^]]|alarm.*active'; then printf 'RABBITMQ_ALARM_OK=true\\n'; else printf 'RABBITMQ_ALARM_OK=false\\n'; fi",
    "local.rabbitmq.queue.backlog": "out=$(<RABBITMQCTL> list_queues -p / name state messages messages_ready messages_unacknowledged consumers memory --formatter=csv 2>&1); rc=$?; if [ \"$rc\" -ne 0 ]; then printf 'RABBITMQ_QUEUE_QUERY_FAILED=true\\n'; else max=$(printf '%s\\n' \"$out\" | awk -F, '{for (i=3;i<=5;i++) {gsub(/[\" ]/,\"\",$i); if (($i+0)>max) max=$i+0}} END {print max+0}'); printf 'RABBITMQ_QUEUE_BACKLOG=%s\\n' \"${max:-0}\"; fi",
    "local.rabbitmq.connection.pressure": "connections=$(<RABBITMQCTL> list_connections name user peer_host state channels send_pend recv_oct send_oct --formatter=csv 2>&1); connections_rc=$?; channels=$(<RABBITMQCTL> list_channels connection number consumer_count messages_unacknowledged --formatter=csv 2>&1); channels_rc=$?; if [ \"$connections_rc\" -ne 0 ] || [ \"$channels_rc\" -ne 0 ]; then printf 'RABBITMQ_CONNECTION_QUERY_FAILED=true\\n'; else max=$(printf '%s\\n' \"$connections\" | awk -F, '{gsub(/[\" ]/,\"\",$6); if (($6+0)>max) max=$6+0} END {print max+0}'); unack=$(printf '%s\\n' \"$channels\" | awk -F, '{gsub(/[\" ]/,\"\",$4); if (($4+0)>max) max=$4+0} END {print max+0}'); [ \"${unack:-0}\" -gt \"${max:-0}\" ] && max=$unack; printf 'RABBITMQ_CONNECTION_PRESSURE=%s\\n' \"${max:-0}\"; fi",
    "local.rabbitmq.error_log": "if [ ! -d <RABBITMQ_LOG> ]; then printf 'RABBITMQ_ERROR_LOG_UNAVAILABLE=true\\n'; else hits=$(grep -R --include='*.log' -iE '\\[error\\]|FATAL|CRASH REPORT|SUPERVISOR REPORT|memory alarm|disk alarm|network partition|handshake failure' <RABBITMQ_LOG> 2>/dev/null | tail -80 | wc -l); printf 'RABBITMQ_ERROR_LOG_HITS=%s\\n' \"$hits\"; fi",
    "local.rabbitmq.config.baseline": "count=$(grep -Ec '^[[:space:]]*(listeners.tcp.default|management.tcp.port|management.tcp.ip|loopback_users.guest|cluster_name|vm_memory_high_watermark.relative|disk_free_limit.absolute|collect_statistics_interval|cluster_partition_handling)[[:space:]]*=' <RABBITMQ_CONF> 2>/dev/null || printf 0); if [ -r <RABBITMQ_CONF> ] && [ \"$count\" -ge 5 ]; then printf 'RABBITMQ_CONFIG_OK=true\\n'; else printf 'RABBITMQ_CONFIG_OK=false\\n'; fi",
    "local.rabbitmq.node.identity": "host=$(hostname -f 2>/dev/null); if [ -r <RABBITMQ_ENV_CONF> ] && [ -r <RABBITMQ_COOKIE> ] && grep -Eq '^(NODENAME|USE_LONGNAME|CONFIG_FILE|MNESIA_BASE|LOG_BASE|PID_FILE)=' <RABBITMQ_ENV_CONF> && [ \"$(stat -c '%U' <RABBITMQ_COOKIE> 2>/dev/null)\" = <RABBITMQ_USER> ] && [ \"$(stat -c '%a' <RABBITMQ_COOKIE> 2>/dev/null)\" = 400 ] && [ \"$(printf '%s\\n' \"$host\" | wc -c)\" -gt 1 ]; then printf 'RABBITMQ_IDENTITY_OK=true\\n'; else printf 'RABBITMQ_IDENTITY_OK=false\\n'; fi",
    ...
    "local.redis.sentinel.health": "if [ \"$redis_mode\" != sentinel ]; then printf 'REDIS_SENTINEL_OK=true\\n'; else info=$(REDISCLI_AUTH=\"\" \"$redis_cli\" -h \"$redis_host\" -p \"$redis_sentinel_port\" --user \"$redis_user\" --no-auth-warning --raw INFO sentinel 2>&1); info_rc=$?; if printf '%s\\n' \"$info\" | grep -Eqi 'NOAUTH|WRONGPASS'; then info=$(\"$redis_cli\" -h \"$redis_host\" -p \"$redis_sentinel_port\" --user \"$redis_user\" --no-auth-warning --raw INFO sentinel 2>&1); info_rc=$?; fi; masters=$(REDISCLI_AUTH=\"\" \"$redis_cli\" -h \"$redis_host\" -p \"$redis_sentinel_port\" --user \"$redis_user\" --no-auth-warning --raw SENTINEL masters 2>&1); masters_rc=$?; if printf '%s\\n' \"$masters\" | grep -Eqi 'NOAUTH|WRONGPASS'; then masters=$(\"$redis_cli\" -h \"$redis_host\" -p \"$redis_sentinel_port\" --user \"$redis_user\" --no-auth-warning --raw SENTINEL masters 2>&1); masters_rc=$?; fi; if [ \"$info_rc\" -ne 0 ] || [ \"$masters_rc\" -ne 0 ] || printf '%s\\n%s\\n' \"$info\" \"$masters\" | grep -Eqi 'NOAUTH|WRONGPASS|connection refused|Could not connect|timeout|ERR '; then printf 'REDIS_SENTINEL_FACT_UNAVAILABLE=true\\n'; else info_master_count=$(printf '%s\\n' \"$info\" | sed -n 's/^sentinel_masters:\\([0-9][0-9]*\\)$/\\1/p'); listed_master_count=$(printf '%s\\n' \"$masters\" | awk 'NR % 2 == 1 && $0 == \"name\" {n++} END {print n+0}'); sentinel_count=$(printf '%s\\n' \"$masters\" | awk 'NR % 2 == 1 {key=$0; next} key == \"name\" {masters++} key == \"num-other-sentinels\" {others += $0+0} END {print others+masters+0}'); bad_flags=$(printf '%s\\n' \"$masters\" | awk 'NR % 2 == 1 {key=$0; next} key == \"flags\" && $0 ~ /(^|,)s_down(,|$)|(^|,)o_down(,|$)|(^|,)disconnected(,|$)/ {bad=1} END {print bad+0}'); if [ \"${info_master_count:-0}\" -ge 1 ] 2>/dev/null && [ \"${listed_master_count:-0}\" -ge \"${info_master_count:-1}\" ] 2>/dev/null && [ \"${sentinel_count:-0}\" -ge \"$redis_expected_sentinels\" ] 2>/dev/null && [ \"${bad_flags:-1}\" -eq 0 ] 2>/dev/null; then printf 'REDIS_SENTINEL_OK=true\\n'; else printf 'REDIS_SENTINEL_OK=false\\n'; fi; fi; fi",
    "local.redis.cluster.health": "if [ \"$redis_mode\" != cluster ]; then printf 'REDIS_CLUSTER_OK=true\\n'; else info=$(\"$redis_cli\" -h \"$redis_host\" -p \"$redis_data_port\" --user \"$redis_user\" --no-auth-warning CLUSTER INFO 2>&1); info_rc=$?; nodes=$(\"$redis_cli\" -h \"$redis_host\" -p \"$redis_data_port\" --user \"$redis_user\" --no-auth-warning CLUSTER NODES 2>&1); nodes_rc=$?; masters=$(printf '%s\\n' \"$nodes\" | awk '$3 ~ /(^|,)master(,|$)/ && $3 !~ /fail|noaddr/ {n++} END {print n+0}'); replicas=$(printf '%s\\n' \"$nodes\" | awk '$3 ~ /(^|,)slave(,|$)/ && $3 !~ /fail|noaddr/ {n++} END {print n+0}'); if [ \"$info_rc\" -ne 0 ] || [ \"$nodes_rc\" -ne 0 ] || printf '%s\\n%s\\n' \"$info\" \"$nodes\" | grep -Eqi 'NOAUTH|WRONGPASS|connection refused|Could not connect|timeout|ERR '; then printf 'REDIS_CLUSTER_FACT_UNAVAILABLE=true\\n'; elif printf '%s\\n' \"$info\" | grep -Eq '^cluster_state:ok$' && printf '%s\\n' \"$info\" | grep -Eq '^cluster_slots_ok:16384$' && [ \"${masters:-0}\" -ge \"$redis_expected_masters\" ] 2>/dev/null && [ \"${replicas:-0}\" -ge \"$redis_expected_replicas\" ] 2>/dev/null && ! printf '%s\\n%s\\n' \"$info\" \"$nodes\" | grep -Eqi 'fail|noaddr|handshake'; then printf 'REDIS_CLUSTER_OK=true\\n'; else printf 'REDIS_CLUSTER_OK=false\\n'; fi; fi",
    ...
    "local.rocketmq.error_log": "if [ -d \"$rocketmq_log_dir\" ]; then hits=$(grep -R -iE '(^|[^[:alnum:]_])(ERROR|FATAL)([^[:alnum:]_]|$)|OutOfMemory|Address already in use|BrokerStartup failed|NamesrvStartup failed' \"$rocketmq_log_dir\" --include='*.log' 2>/dev/null | tail -100 | wc -l); printf 'ROCKETMQ_ERROR_LOG_HITS=%s\\n' \"$hits\"; else printf 'ROCKETMQ_ERROR_LOG_UNAVAILABLE=true\\n'; fi",
    "local.rocketmq.config.baseline": "if [ -r \"$rocketmq_broker_conf\" ] && [ -r \"$rocketmq_namesrv_conf\" ] && grep -Eq '(^|[[:space:]])(brokerClusterName|brokerName|brokerId|namesrvAddr|listenPort|haListenPort|storePathRootDir|autoCreateTopicEnable)[[:space:]]*[=:]' \"$rocketmq_broker_conf\" && grep -Eq '(^|[[:space:]])(listenPort|kvConfigPath|configStorePath)[[:space:]]*[=:]' \"$rocketmq_namesrv_conf\"; then printf 'ROCKETMQ_CONFIG_OK=true\\n'; else printf 'ROCKETMQ_CONFIG_OK=false\\n'; fi",
    ...
    # Tomcat access-log naming depends on the AccessLogValve configuration:
    # support both the manual's localhost_access_log*.txt and the common
    # access.YYYY-MM-DD.log / localhost.YYYY-MM-DD.log forms.
    "local.tomcat.access_log.5xx": "total=0; found=0; for file in \"$tomcat_log_dir\"/localhost_access_log*.txt \"$tomcat_log_dir\"/access*.log \"$tomcat_log_dir\"/localhost*.log; do if [ -f \"$file\" ]; then found=1; n=$(tail -1000 \"$file\" | awk '$9 ~ /^5/ {c++} END {print c+0}'); total=$((total+n)); fi; done; if [ \"$found\" -eq 1 ]; then printf 'TOMCAT_ACCESS_5XX=%s\\n' \"$total\"; else printf 'TOMCAT_ACCESS_LOG_UNAVAILABLE=true\\n'; fi",
    ...
    "local.tomcat.port.isolation": "listeners=0; bad=0; for p in \"$tomcat_port\" \"$tomcat_shutdown_port\"; do line=$(ss -H -lntp 2>/dev/null | grep -E :$p'[[:space:]]'); if [ -n \"$line\" ]; then listeners=$((listeners+1)); if ! printf '%s\\n' \"$line\" | grep -q '\"java\"'; then bad=$((bad+1)); fi; fi; done; if [ \"$tomcat_https_enabled\" = true ]; then line=$(ss -H -lntp 2>/dev/null | grep -E :$tomcat_https_port'[[:space:]]'); if [ -n \"$line\" ]; then listeners=$((listeners+1)); if ! printf '%s\\n' \"$line\" | grep -q '\"java\"'; then bad=$((bad+1)); fi; fi; fi; if [ -r \"$tomcat_conf_file\" ] && [ \"$listeners\" -gt 0 ] 2>/dev/null && [ \"$bad\" -eq 0 ] 2>/dev/null; then printf 'TOMCAT_PORT_ISOLATION_OK=true\\n'; else printf 'TOMCAT_PORT_ISOLATION_OK=false\\n'; fi",
}
...
_ADDITIONAL_PLACEHOLDER_KEYS = {
    ...
    "local.rabbitmq.service.health": {
        "<RABBITMQ_CONF>": "rabbitmq_conf", "<RABBITMQ_BIN>": "rabbitmq_bin", "<RABBITMQ_LOG>": "rabbitmq_log", "<RABBITMQ_UNIT>": "rabbitmq_unit",
    },
    "local.rabbitmq.node.health": {"<RABBITMQ_DIAGNOSTICS>": ("rabbitmq_bin", "rabbitmq-diagnostics")},
    "local.rabbitmq.core_ports.health": {
        "<RABBITMQ_PORT>": "rabbitmq_port", "<RABBITMQ_MANAGEMENT_PORT>": "rabbitmq_management_port",
        "<RABBITMQ_CLUSTER_PORT>": "rabbitmq_cluster_port", "<RABBITMQ_EPMD_PORT>": "rabbitmq_epmd_port",
    },
    ...
    "local.rabbitmq.error_log": {"<RABBITMQ_LOG>": "rabbitmq_log"},
    "local.rabbitmq.config.baseline": {"<RABBITMQ_CONF>": "rabbitmq_conf"},
    "local.rabbitmq.node.identity": {
        "<RABBITMQ_ENV_CONF>": "rabbitmq_env_conf", "<RABBITMQ_COOKIE>": "rabbitmq_cookie", "<RABBITMQ_USER>": "rabbitmq_user",
    },
    "local.rabbitmq.security.permissions": {"<RABBITMQCTL>": ("rabbitmq_bin", "rabbitmqctl")},
    "local.rabbitmq.topology": {"<RABBITMQCTL>": ("rabbitmq_bin", "rabbitmqctl")},
    "local.rabbitmq.queue.durability": {"<RABBITMQCTL>": ("rabbitmq_bin", "rabbitmqctl")},
    "local.rabbitmq.file_descriptor.limits": {},
    "local.rabbitmq.systemd.unit": {"<RABBITMQ_UNIT>": "rabbitmq_unit"},
    "local.rabbitmq.log.data.retention": {
        "<RABBITMQ_LOG>": "rabbitmq_log", "<RABBITMQ_DATA>": "rabbitmq_data",
    },
    "local.redis.service.health": {
        "<REDIS_BIN>": "redis_bin", "<REDIS_CONF>": "redis_conf",
    },
    "local.redis.ping.version": {},
    "local.redis.core_ports.health": {
        "<REDIS_PORT>": "redis_port", "<REDIS_REPLICA_PORT>": "redis_replica_port",
        "<REDIS_SENTINEL_PORT>": "redis_sentinel_port", "<REDIS_CLUSTER_PORT>": "redis_cluster_port",
        "<REDIS_CLUSTER_BUS_PORT>": "redis_cluster_bus_port",
    },
    "local.redis.replication.health": {},
    "local.redis.sentinel.health": {"<REDIS_SENTINEL_PORT>": "redis_sentinel_port"},
    "local.redis.cluster.health": {"<REDIS_CLUSTER_PORT>": "redis_cluster_port"},
    "local.redis.memory.pressure": {},
    "local.redis.persistence.health": {},
    "local.redis.error_log": {"<REDIS_LOG>": "redis_log"},
    "local.redis.config.baseline": {"<REDIS_CONF>": "redis_conf"},
    "local.redis.security.baseline": {"<REDIS_CONF>": "redis_conf"},
...
def _rocketmq_runtime_prefix(profile: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"rocketmq_expected_sync_replicas={shlex.quote(_additional_profile_value(profile, 'rocketmq_expected_sync_replicas'))}; "
        "detected_namesrv=$(awk -F= '$1 ~ /^[[:space:]]*namesrvAddr[[:space:]]*$/ {gsub(/^[[:space:]]+|[[:space:]]+$/, \"\", $2); print $2; exit}' \"$rocketmq_broker_conf\"); "
...
def _redis_task_environment(profile: Dict[str, Any]) -> Dict[str, str]:
    """Pass Redis ACL/password credentials through Ansible's private environment."""
    def first_value(name: str) -> str:
        values = profile.get(name) or []
        if isinstance(values, str):
            values = [values]
...
def build_metric_command_specs(
    ...
    for m in metrics:
        ...
        if metric_id in _ADDITIONAL_COMMANDS:
            ...
            elif metric_id.startswith("local.redis."):
                task_environment = _redis_task_environment(profile)
            specs.append(
                CommandSpec(
                    metric_id=metric_id,
                    command=command,
...
def select_middleware_metrics(
    ...
    for item in results:
        ...
        if metric_id == "local.tomcat.service.health" and re.search(
            r"TOMCAT_PROCESS_PRESENT=false", stdout, re.IGNORECASE
        ):
            stopped_modules.add("tomcat")
        match = re.search(
