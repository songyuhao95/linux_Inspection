...
_ADDITIONAL_COMMANDS = {
    ...
    "local.redis.sentinel.health": "if [ \"$redis_mode\" != sentinel ]; then printf 'REDIS_SENTINEL_OK=true\\n'; else info=$(\"$redis_cli\" -h \"$redis_host\" -p \"$redis_sentinel_port\" --user \"$redis_user\" --no-auth-warning INFO sentinel 2>&1); info_rc=$?; masters=$(\"$redis_cli\" -h \"$redis_host\" -p \"$redis_sentinel_port\" --user \"$redis_user\" --no-auth-warning SENTINEL masters 2>&1); masters_rc=$?; if [ \"$info_rc\" -ne 0 ] || [ \"$masters_rc\" -ne 0 ] || printf '%s\\n%s\\n' \"$info\" \"$masters\" | grep -Eqi 'NOAUTH|WRONGPASS|connection refused|Could not connect|timeout|ERR '; then printf 'REDIS_SENTINEL_FACT_UNAVAILABLE=true\\n'; elif printf '%s\\n%s\\n' \"$info\" \"$masters\" | grep -Eq 'sentinel_masters:1' && printf '%s\\n%s\\n' \"$info\" \"$masters\" | grep -Eq 'num-slaves[^0-9]*2' && ! printf '%s\\n%s\\n' \"$info\" \"$masters\" | grep -Eqi 's_down|o_down|disconnected'; then printf 'REDIS_SENTINEL_OK=true\\n'; else printf 'REDIS_SENTINEL_OK=false\\n'; fi; fi",
