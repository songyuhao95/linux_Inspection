...
_ADDITIONAL_COMMANDS = {
    ...
    "local.rabbitmq.service.health": "if test -r <RABBITMQ_CONF> && test -x <RABBITMQ_BIN> && pgrep -fa 'beam.smp|rabbitmq-server' >/dev/null 2>&1 && [ \"$(systemctl is-active <RABBITMQ_UNIT> 2>/dev/null)\" = active ]; then printf 'RABBITMQ_SERVICE_OK=true\\n'; else printf 'RABBITMQ_SERVICE_OK=false\\n'; fi",
    ...
    "local.rabbitmq.queue.backlog": "out=$(<RABBITMQCTL> list_queues -p / name state messages messages_ready messages_unacknowledged consumers memory --formatter=tsv 2>&1); rc=$?; if [ \"$rc\" -ne 0 ]; then printf 'RABBITMQ_QUEUE_QUERY_FAILED=true\\n'; else max=$(printf '%s\\n' \"$out\" | awk 'NR>1 {for (i=3;i<=5;i++) if (($i+0)>max) max=$i+0} END {print max+0}'); printf 'RABBITMQ_QUEUE_BACKLOG=%s\\n' \"${max:-0}\"; fi",
    "local.rabbitmq.connection.pressure": "connections=$(<RABBITMQCTL> list_connections name user peer_host state channels send_pend recv_oct send_oct --formatter=tsv 2>&1); connections_rc=$?; channels=$(<RABBITMQCTL> list_channels connection number consumer_count messages_unacknowledged --formatter=tsv 2>&1); channels_rc=$?; if [ \"$connections_rc\" -ne 0 ] || [ \"$channels_rc\" -ne 0 ]; then printf 'RABBITMQ_CONNECTION_QUERY_FAILED=true\\n'; else max=$(printf '%s\\n' \"$connections\" | awk 'NR>1 && ($6+0)>max {max=$6+0} END {print max+0}'); unack=$(printf '%s\\n' \"$channels\" | awk 'NR>1 && ($4+0)>max {max=$4+0} END {print max+0}'); [ \"${unack:-0}\" -gt \"${max:-0}\" ] && max=$unack; printf 'RABBITMQ_CONNECTION_PRESSURE=%s\\n' \"${max:-0}\"; fi",
    "local.rabbitmq.error_log": "if [ ! -d <RABBITMQ_LOG> ]; then printf 'RABBITMQ_ERROR_LOG_UNAVAILABLE=true\\n'; else hits=$(grep -R -iE 'ERROR|FATAL|CRASH REPORT|SUPERVISOR REPORT|memory alarm|disk alarm|partition|Mnesia|epmd|connection.*closed|handshake' <RABBITMQ_LOG> 2>/dev/null | tail -80 | wc -l); printf 'RABBITMQ_ERROR_LOG_HITS=%s\\n' \"$hits\"; fi",
    "local.rabbitmq.config.baseline": "count=$(grep -Ec '^(listeners.tcp.default|management.tcp.port|management.tcp.ip|loopback_users.guest|cluster_name|vm_memory_high_watermark.relative|disk_free_limit.absolute|collect_statistics_interval|cluster_partition_handling)=' <RABBITMQ_CONF> 2>/dev/null || printf 0); if [ -r <RABBITMQ_CONF> ] && [ \"$count\" -ge 5 ]; then printf 'RABBITMQ_CONFIG_OK=true\\n'; else printf 'RABBITMQ_CONFIG_OK=false\\n'; fi",
    "local.rabbitmq.node.identity": "host=$(hostname -f 2>/dev/null); if [ -r <RABBITMQ_ENV_CONF> ] && [ -r <RABBITMQ_COOKIE> ] && grep -Eq '^(NODENAME|USE_LONGNAME|CONFIG_FILE|MNESIA_BASE|LOG_BASE|PID_FILE)=' <RABBITMQ_ENV_CONF> && [ \"$(stat -c '%U' <RABBITMQ_COOKIE> 2>/dev/null)\" = rabbitmq ] && [ \"$(stat -c '%a' <RABBITMQ_COOKIE> 2>/dev/null)\" = 400 ] && [ \"$(printf '%s\\n' \"$host\" | wc -c)\" -gt 1 ]; then printf 'RABBITMQ_IDENTITY_OK=true\\n'; else printf 'RABBITMQ_IDENTITY_OK=false\\n'; fi",
    ...
    "local.rabbitmq.topology": "vhosts=$(<RABBITMQCTL> list_vhosts 2>&1); vhosts_rc=$?; exchanges=$(<RABBITMQCTL> list_exchanges -p / name type durable auto_delete --formatter=tsv 2>&1); exchanges_rc=$?; bindings=$(<RABBITMQCTL> list_bindings -p / source_name destination_name destination_kind routing_key --formatter=tsv 2>&1); bindings_rc=$?; if [ \"$vhosts_rc\" -eq 0 ] && [ \"$exchanges_rc\" -eq 0 ] && [ \"$bindings_rc\" -eq 0 ]; then printf 'RABBITMQ_TOPOLOGY_OK=true\\n'; else printf 'RABBITMQ_TOPOLOGY_OK=false\\n'; fi",
    "local.rabbitmq.queue.durability": "out=$(<RABBITMQCTL> list_queues -p / name durable auto_delete exclusive arguments consumers messages_ready messages_unacknowledged --formatter=tsv 2>&1); rc=$?; if [ \"$rc\" -ne 0 ]; then printf 'RABBITMQ_QUEUE_DURABILITY_OK=false\\n'; elif printf '%s\\n' \"$out\" | awk 'NR>1 && $2 != \"true\" {bad=1} END {exit bad}'; then printf 'RABBITMQ_QUEUE_DURABILITY_OK=true\\n'; else printf 'RABBITMQ_QUEUE_DURABILITY_OK=false\\n'; fi",
...
_ADDITIONAL_PLACEHOLDER_KEYS = {
    ...
    "local.rabbitmq.node.identity": {
        "<RABBITMQ_ENV_CONF>": "rabbitmq_env_conf", "<RABBITMQ_COOKIE>": "rabbitmq_cookie",
    },
