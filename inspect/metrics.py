...
ELASTICSEARCH_METRICS = [
    ...
    _es_metric("local.elasticsearch.snapshot.repository", "快照仓库",
                ...
                "存在已注册仓库且仓库配置可读 → OK；仓库缺失或配置读取失败 → WARN/CRIT",
...
def _middleware_metric(
    ...
    *,
    ...
    numeric_units = {
        ...
        "local.rabbitmq.core_ports.health": "端口数",
        "local.rabbitmq.queue.backlog": "消息数",
        "local.rabbitmq.connection.pressure": "数量",
        "local.rabbitmq.error_log": "条数",
        "local.rabbitmq.file_descriptor.limits": "文件句柄数",
        "local.rabbitmq.log.data.retention": "文件数",
        "local.redis.core_ports.health": "端口数",
        "local.redis.memory.pressure": "百分比",
        "local.redis.error_log": "条数",
        "local.redis.slow_query": "条数",
        "local.redis.clients.pressure": "连接数",
        "local.redis.keyspace.stats": "键数",
        "local.redis.system.parameters": "文件句柄数",
        "local.redis.log.data.retention": "文件数",
        "local.rocketmq.consumer.lag": "消息数",
        "local.rocketmq.core_ports.health": "端口数",
        "local.rocketmq.jvm.memory": "百分比",
        "local.rocketmq.error_log": "条数",
        "local.rocketmq.broker.runtime": "队列数",
        "local.rocketmq.jvm.gc": "次数",
...
_ADDITIONAL_MIDDLEWARE_METRICS = [
    ...
    _middleware_metric("local.rabbitmq.service.health", "RabbitMQ 服务健康", "pgrep -fa 'beam.smp|rabbitmq-server'", "Rabbitmq运维巡检手册v1.0.docx", "P0-TABLE5", "systemd active 且 beam.smp/RabbitMQ 进程存在，运行用户为 rabbitmq → OK；否则 → CRIT"),
    _middleware_metric("local.rabbitmq.node.health", "RabbitMQ 节点健康", "/opt/rabbitmq/sbin/rabbitmq-diagnostics ping\n/opt/rabbitmq/sbin/rabbitmq-diagnostics status", "Rabbitmq运维巡检手册v1.0.docx", "P0-TABLE5", "Ping succeeded 且 status 可返回节点、版本、Erlang 和运行时信息 → OK；失败或超时 → CRIT"),
    _middleware_metric("local.rabbitmq.core_ports.health", "RabbitMQ 核心端口", "ss -tlnp | egrep ':5672|:15672|:25672|:4369'", "Rabbitmq运维巡检手册v1.0.docx", "P0-TABLE5", "5672、15672、25672、4369 均 LISTEN → OK；业务、管理或集群端口缺失 → WARN/CRIT"),
    _middleware_metric("local.rabbitmq.cluster.nodes", "RabbitMQ 集群节点", "source /home/rabbitmq/.bash_profile\n/opt/rabbitmq/sbin/rabbitmqctl cluster_status", "Rabbitmq运维巡检手册v1.0.docx", "P0-TABLE5", "running_nodes 包含规划 3 节点且 cluster_name 为 rabbitmq-cluster → OK；少于 3 → WARN；少于 2 → CRIT"),
    _middleware_metric("local.rabbitmq.alarm.partition", "RabbitMQ 告警与分区", "/opt/rabbitmq/sbin/rabbitmq-diagnostics check_local_alarms\n/opt/rabbitmq/sbin/rabbitmqctl cluster_status | egrep -i 'alarms|partitions|running_nodes'", "Rabbitmq运维巡检手册v1.0.docx", "P0-TABLE5", "无 memory/disk alarm 且 partitions 为空 → OK；出现告警或网络分区 → CRIT"),
    _middleware_metric("local.rabbitmq.queue.backlog", "RabbitMQ 队列积压", "/opt/rabbitmq/sbin/rabbitmqctl list_queues -p / name state messages messages_ready messages_unacknowledged consumers memory --formatter=pretty_table", "Rabbitmq运维巡检手册v1.0.docx", "P0-TABLE5", "队列 state 为 running，积压消息不超过 100 条为 OK，100-1000 为 WARN，超过 1000 为 CRIT"),
    _middleware_metric("local.rabbitmq.connection.pressure", "RabbitMQ 连接与信道压力", "/opt/rabbitmq/sbin/rabbitmqctl list_connections name user peer_host state channels send_pend recv_oct send_oct --formatter=pretty_table\n/opt/rabbitmq/sbin/rabbitmqctl list_channels connection number consumer_count messages_unacknowledged --formatter=pretty_table", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "无 blocked/closing/flow 状态且 send_pend 与未确认消息不堆积，100 以内 OK，100-1000 WARN，超过 1000 CRIT"),
    _middleware_metric("local.rabbitmq.error_log", "RabbitMQ 关键错误日志", "grep -R -iE 'ERROR|FATAL|CRASH REPORT|SUPERVISOR REPORT|memory alarm|disk alarm|partition|Mnesia|epmd|connection.*closed|handshake' /opt/rabbitmq/logs 2>/dev/null | tail -80", "Rabbitmq运维巡检手册v1.0.docx", "P0-TABLE5", "无 ERROR/FATAL、Mnesia、连接、认证和资源告警，0 条 OK，1-10 条 WARN，超过 10 条 CRIT"),
    _middleware_metric("local.rabbitmq.config.baseline", "RabbitMQ 配置基线", "grep -E '^(listeners.tcp.default|management.tcp.port|management.tcp.ip|loopback_users.guest|cluster_name|vm_memory_high_watermark.relative|disk_free_limit.absolute|collect_statistics_interval|cluster_partition_handling)' /opt/rabbitmq/conf/rabbitmq.conf", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "端口、管理 IP、cluster_name、内存/磁盘阈值、统计周期和分区策略符合部署基线 → OK；配置漂移 → CRIT"),
    _middleware_metric("local.rabbitmq.node.identity", "RabbitMQ 节点身份与 Cookie", "hostname -f\ngrep -E '^(NODENAME|USE_LONGNAME|CONFIG_FILE|MNESIA_BASE|LOG_BASE|PID_FILE)' /opt/rabbitmq/conf/rabbitmq-env.conf\nls -l /home/rabbitmq/.erlang.cookie\ncat /etc/hosts | egrep 'kylin01|kylin02|kylin03'", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "hostname、NODENAME、USE_LONGNAME、Cookie 权限和 hosts 集群解析一致 → OK；漂移或权限异常 → CRIT"),
    _middleware_metric("local.rabbitmq.security.permissions", "RabbitMQ 用户与权限", "source /home/rabbitmq/.bash_profile\n/opt/rabbitmq/sbin/rabbitmqctl list_users\n/opt/rabbitmq/sbin/rabbitmqctl list_permissions -p /", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "管理员和业务用户权限符合授权基线，guest 未开放远程访问 → OK；未知管理员、权限过大或 guest 暴露 → CRIT"),
    _middleware_metric("local.rabbitmq.topology", "RabbitMQ VHost 与拓扑", "source /home/rabbitmq/.bash_profile\n/opt/rabbitmq/sbin/rabbitmqctl list_vhosts\n/opt/rabbitmq/sbin/rabbitmqctl list_exchanges -p / name type durable auto_delete --formatter=pretty_table\n/opt/rabbitmq/sbin/rabbitmqctl list_bindings -p / source_name destination_name destination_kind routing_key --formatter=pretty_table", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "VHost、Exchange、Binding 与业务登记一致，关键 Exchange 持久化 → OK；缺失或异常 → CRIT"),
    _middleware_metric("local.rabbitmq.queue.durability", "RabbitMQ 队列持久化", "source /home/rabbitmq/.bash_profile\n/opt/rabbitmq/sbin/rabbitmqctl list_queues -p / name durable auto_delete exclusive arguments consumers messages_ready messages_unacknowledged --formatter=pretty_table", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "关键业务队列 durable=true，auto_delete/exclusive 与业务设计一致，积压可控 → OK；不符合 → CRIT"),
    _middleware_metric("local.rabbitmq.file_descriptor.limits", "RabbitMQ 文件句柄与进程限制", "PID=$(pgrep -f 'beam.smp' | head -1); test -n \"$PID\" && grep -E 'Max open files|Max processes' /proc/$PID/limits\nsu - rabbitmq -c 'ulimit -n; ulimit -u'\nsystemctl show rabbitmq -p LimitNOFILE -p LimitNPROC", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "nofile>=65535、nproc>=4096 且 systemd 限制符合基线 → OK；资源限制不足 → CRIT"),
    _middleware_metric("local.rabbitmq.systemd.unit", "RabbitMQ systemd 服务", "systemctl cat rabbitmq | egrep 'User=|Group=|WorkingDirectory=|ExecStart=|ExecStop=|Restart=|LimitNOFILE|LimitNPROC|Environment='", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "rabbitmq.service 的 User/Group、启动停止命令、自动重启和限制符合部署规范 → OK；配置漂移 → CRIT"),
    _middleware_metric("local.rabbitmq.log.data.retention", "RabbitMQ 日志与数据目录留存", "du -sh /opt/rabbitmq/data /opt/rabbitmq/logs 2>/dev/null\nfind /opt/rabbitmq/logs -type f -mtime +30 -ls 2>/dev/null | head -50\nfind /opt/rabbitmq/data -maxdepth 3 -type f -size +500M -ls 2>/dev/null | head -50", "Rabbitmq运维巡检手册v1.0.docx", "P1-TABLE6", "日志和数据目录增长平稳，无大量历史日志和异常大文件；0-50 个历史文件 OK，51-100 WARN，超过 100 CRIT"),

    _middleware_metric("local.redis.service.health", "Redis 服务健康", "pgrep -fa 'redis-server.*(6379|16379|7000)'", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "redis-server 进程存在且使用规划配置，运行用户符合部署规范 → OK；进程缺失或路径漂移 → CRIT"),
    _middleware_metric("local.redis.ping.version", "Redis 登录与版本", "export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=6379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning PING; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO server | egrep 'redis_version|tcp_port|process_id|uptime_in_seconds'", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "返回 PONG，redis_version 符合 7.4.2，tcp_port 与部署模式一致 → OK；认证失败、拒绝连接、超时或版本漂移 → CRIT"),
    _middleware_metric("local.redis.core_ports.health", "Redis 核心端口", "ss -tlnp | egrep ':6379|:16379|:26379|:7000|:17000'", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "按 standalone/Sentinel/Cluster 模式检查业务、Sentinel、Cluster 和 bus 端口 → OK；关键端口缺失 → CRIT"),
    _middleware_metric("local.redis.replication.health", "Redis 复制健康", "export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=16379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO replication", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "角色、从库数和 master_link_status=up 符合规划 → OK；复制链路中断或角色异常 → CRIT"),
    _middleware_metric("local.redis.sentinel.health", "Redis Sentinel 健康", "export REDISCLI_AUTH='<REDIS_AUTH>'; /opt/redis/bin/redis-cli -h 127.0.0.1 -p 26379 --no-auth-warning INFO sentinel; /opt/redis/bin/redis-cli -h 127.0.0.1 -p 26379 --no-auth-warning SENTINEL masters", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "Sentinel 模式下 master、slave、其他 Sentinel 数量和状态符合规划且无 s_down/o_down/disconnected → OK；否则 → CRIT；非 Sentinel 模式不适用"),
    _middleware_metric("local.redis.cluster.health", "Redis Cluster 健康", "export REDISCLI_AUTH='<REDIS_AUTH>'; /opt/redis/bin/redis-cli -h 127.0.0.1 -p 7000 --no-auth-warning CLUSTER INFO; /opt/redis/bin/redis-cli -h 127.0.0.1 -p 7000 --no-auth-warning CLUSTER NODES", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "Cluster 模式下 cluster_state=ok、16384/16384 slots、3 master+3 slave 且无 fail/noaddr/handshake → OK；否则 → CRIT；非 Cluster 模式不适用"),
    _middleware_metric("local.redis.memory.pressure", "Redis 内存水位与淘汰", "export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=6379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO memory | egrep 'used_memory_human|used_memory_peak_human|maxmemory_human|maxmemory_policy|mem_fragmentation_ratio|mem_not_counted_for_evict'; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO stats | egrep 'evicted_keys|rejected_connections|total_error_replies'", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "used_memory/maxmemory 百分比 ≤80% 且无异常淘汰、拒绝连接 → OK；80%-95% → WARN；>95% 或持续淘汰/拒绝 → CRIT"),
    _middleware_metric("local.redis.persistence.health", "Redis 持久化健康", "export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=6379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO persistence | egrep 'loading|rdb_last_bgsave_status|rdb_last_bgsave_time_sec|rdb_changes_since_last_save|aof_enabled|aof_last_bgrewrite_status|aof_last_write_status|aof_current_size|aof_base_size'; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning CONFIG GET appendonly appendfsync save dir dbfilename appendfilename", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "loading=0、RDB/AOF 状态正常且 appendfsync=everysec → OK；持久化失败或加载中 → CRIT"),
    _middleware_metric("local.redis.error_log", "Redis 关键错误日志", "grep -R -iE 'WARNING|ERROR|OOM|MISCONF|Background saving error|AOF|RDB|READONLY|MASTERDOWN|CLUSTERDOWN|fail|denied|invalid password' /opt/redis/logs 2>/dev/null | tail -80", "Redis运维巡检手册v1.0.docx", "P0-TABLE5", "无 OOM、MISCONF、AOF/RDB 失败、MASTERDOWN、CLUSTERDOWN 或认证失败 → OK；关键错误 1-10 条 WARN，超过 10 条 CRIT"),
    _middleware_metric("local.redis.config.baseline", "Redis 配置基线", "grep -E '^(bind|protected-mode|port|tcp-backlog|daemonize|supervised|pidfile|logfile|dir|dbfilename|appendonly|appendfilename|appendfsync|maxmemory|maxmemory-policy|requirepass|masterauth|replicaof|cluster-enabled|cluster-config-file|cluster-node-timeout|rename-command)' /opt/redis/conf/redis-*.conf", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "绑定地址、端口、目录、AOF/RDB、maxmemory、认证、复制/Cluster 参数符合部署基线 → OK；关键配置漂移 → CRIT"),
    _middleware_metric("local.redis.security.baseline", "Redis 安全与危险命令", "grep -E '^(requirepass|masterauth|protected-mode|rename-command)' /opt/redis/conf/redis-*.conf; export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=6379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning COMMAND INFO FLUSHDB FLUSHALL KEYS", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "requirepass/masterauth、protected-mode 和危险命令控制符合安全基线 → OK；未认证、protected-mode 关闭或危险命令暴露 → CRIT"),
    _middleware_metric("local.redis.slow_query", "Redis 慢查询与延迟", "export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=6379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning SLOWLOG LEN; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning SLOWLOG GET 10; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning LATENCY LATEST", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "slowlog 数量和延迟记录受控 → OK；慢查询持续增加或出现明显延迟 → WARN/CRIT"),
    _middleware_metric("local.redis.clients.pressure", "Redis 客户端连接压力", "export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=6379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO clients | egrep 'connected_clients|blocked_clients|tracking_clients|clients_in_timeout_table'; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning CLIENT LIST | head -30", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "connected_clients 符合容量、blocked_clients=0 且无异常来源连接 → OK；连接接近上限或阻塞 → WARN/CRIT"),
    _middleware_metric("local.redis.keyspace.stats", "Redis Keyspace 统计", "export REDISCLI_AUTH='<REDIS_AUTH>'; REDIS_PORT=6379; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO keyspace; /opt/redis/bin/redis-cli -h 127.0.0.1 -p ${REDIS_PORT} --no-auth-warning INFO stats | egrep 'keyspace_hits|keyspace_misses|expired_keys|evicted_keys'", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "keys、expires、avg_ttl 符合业务预期，命中率和淘汰趋势正常 → OK；misses/expired/evicted 异常增长 → WARN/CRIT"),
    _middleware_metric("local.redis.system.parameters", "Redis 系统参数", "sysctl vm.overcommit_memory net.core.somaxconn; su - redis -c 'ulimit -n; ulimit -u'; systemctl show redis -p LimitNOFILE -p LimitNPROC", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "vm.overcommit_memory=1、somaxconn=65535、Redis 用户 nofile/nproc 和 systemd 限制符合基线 → OK；参数不足 → WARN/CRIT"),
    _middleware_metric("local.redis.service.unit", "Redis 服务单元", "systemctl is-enabled redis; systemctl cat redis | egrep 'User=|Group=|ExecStart=|ExecStop=|LimitNOFILE|Restart='; systemctl is-active redis-sentinel 2>/dev/null; systemctl cat redis-sentinel 2>/dev/null | egrep 'User=|Group=|ExecStart=|ExecStop|LimitNOFILE|Restart='", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "redis.service/redis-sentinel.service 的启动路径、用户、自动重启和限制符合部署模式 → OK；服务单元缺失或漂移 → CRIT"),
    _middleware_metric("local.redis.log.data.retention", "Redis 日志与数据留存", "ls -lh /opt/redis/data | egrep 'dump|appendonly|nodes-|aof'; find /opt/redis/logs -type f -mtime +30 -ls 2>/dev/null | head -50; du -sh /opt/redis/data /opt/redis/logs 2>/dev/null", "Redis运维巡检手册v1.0.docx", "P1-TABLE6", "RDB/AOF/nodes 文件存在且可读，历史日志受控，数据和日志目录增长平稳 → OK；文件缺失或历史日志堆积 → WARN/CRIT"),

    _middleware_metric("local.rocketmq.java.environment", "RocketMQ JDK 与运行环境", "su - rocketmq -c 'source /home/rocketmq/.bash_profile; echo JAVA_HOME=$JAVA_HOME; echo ROCKETMQ_HOME=$ROCKETMQ_HOME; java -version; javac -version; which java; which javac 2>&1'", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "JAVA_HOME=/opt/jdk1.8.0_421、ROCKETMQ_HOME=/opt/rocketmq，java/javac 版本为 1.8.0_421 → OK；缺失或版本不符 → CRIT"),
    _middleware_metric("local.rocketmq.namesrv.health", "RocketMQ NameServer 健康", "ps -ef | grep -E 'NamesrvStartup|mqnamesrv'; tail -n 50 /opt/rocketmq/logs/rocketmqlogs/namesrv.log", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "NameServer 进程存在且日志无启动失败 → OK；进程不存在或启动失败 → CRIT"),
    _middleware_metric("local.rocketmq.broker.health", "RocketMQ Broker 健康", "ps -ef | grep -E 'BrokerStartup|mqbroker'; tail -n 50 /opt/rocketmq/logs/rocketmqlogs/broker.log", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "Broker 进程存在且日志无持续 ERROR/FATAL → OK；进程不存在或启动失败 → CRIT"),
    _middleware_metric("local.rocketmq.core_ports.health", "RocketMQ 核心端口", "ss -lntp | egrep ':9876|:9877|:10911|:10912'", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "集群模式 9876/9877/10911/10912 按配置监听；非 Controller 模式不检查 9877 → OK；关键端口缺失 → CRIT"),
    _middleware_metric("local.rocketmq.cluster.registration", "RocketMQ Broker 注册与集群视图", "source /home/rocketmq/.bash_profile; NAMESRV_ADDR=$(grep -E '^namesrvAddr=' /opt/rocketmq/conf/broker.conf | cut -d= -f2); mqadmin clusterList -n $NAMESRV_ADDR", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "NameServer 返回集群视图并包含注册 Broker → OK；命令失败或无 Broker → UNKNOWN/CRIT"),
    _middleware_metric("local.rocketmq.controller.sync_set", "RocketMQ Controller 与同步集合", "source /home/rocketmq/.bash_profile; CONTROLLER_ADDR=$(grep -E '^controllerAddr=' /opt/rocketmq/conf/broker.conf | cut -d= -f2); mqadmin getControllerMetaData -a $CONTROLLER_ADDR; mqadmin getSyncStateSet -a $CONTROLLER_ADDR -b broker-a", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "Controller 有 leader 且同步集合达到多数派 → OK；无 leader 或同步副本不足 → CRIT"),
    _middleware_metric("local.rocketmq.consumer.lag", "RocketMQ 消息堆积与消费进度", "source /home/rocketmq/.bash_profile; mqadmin consumerProgress -n $NAMESRV_ADDR; mqadmin statsAll -n $NAMESRV_ADDR", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "最大消费 diff/lag ≤100 → OK，101-1000 → WARN，>1000 → CRIT；命令不可用 → UNKNOWN"),
    _middleware_metric("local.rocketmq.jvm.memory", "RocketMQ 内存与 JVM 水位", "free -m; jps -lv; ps -ef | grep -E 'NamesrvStartup|BrokerStartup'", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "主机内存使用率 ≤80% → OK，80%-90% → WARN，>90% → CRIT；JVM 进程不可读 → UNKNOWN"),
    _middleware_metric("local.rocketmq.storage.health", "RocketMQ 磁盘与存储目录", "df -h; df -i; du -sh /opt/rocketmq/store /opt/rocketmq/logs; grep -E 'storePathRootDir|storePathCommitLog|storePathConsumeQueue' /opt/rocketmq/conf/broker.conf", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "存储目录和配置文件可读且文件系统正常 → OK；目录缺失或不可读 → CRIT"),
    _middleware_metric("local.rocketmq.error_log", "RocketMQ 关键日志", "grep -R -iE 'ERROR|FATAL|OutOfMemory|Address already in use|BrokerStartup failed|NamesrvStartup failed' /opt/rocketmq/logs | tail -n 100", "Rocketmq运维巡检手册v1.0.docx", "P0-TABLE5", "关键错误 0 条 → OK，1-10 条 → WARN，>10 条 → CRIT；日志不可读 → UNKNOWN"),
    _middleware_metric("local.rocketmq.config.baseline", "RocketMQ 配置基线", "grep -E 'brokerClusterName|brokerName|brokerId|namesrvAddr|listenPort|haListenPort|storePathRootDir|autoCreateTopicEnable' /opt/rocketmq/conf/broker.conf; grep -E 'listenPort|kvConfigPath|configStorePath' /opt/rocketmq/conf/namesrv.properties", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "Broker/NameServer 核心配置项存在且路径、端口、集群参数符合基线 → OK；缺项或漂移 → CRIT"),
    _middleware_metric("local.rocketmq.topic.route", "RocketMQ Topic 路由与队列分布", "source /home/rocketmq/.bash_profile; mqadmin topicList -n $NAMESRV_ADDR; mqadmin topicRoute -n $NAMESRV_ADDR -t <TOPIC>", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "Topic 列表和路由可查询，队列、副本分布正常 → OK；NameServer 查询失败 → UNKNOWN"),
    _middleware_metric("local.rocketmq.consumer.groups", "RocketMQ 消费组与订阅关系", "source /home/rocketmq/.bash_profile; mqadmin consumerProgress -n $NAMESRV_ADDR; mqadmin consumerStatus -n $NAMESRV_ADDR -g <GROUP>", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "消费组进度和订阅关系可查询，无异常堆积或失联 → OK；查询失败 → UNKNOWN"),
    _middleware_metric("local.rocketmq.broker.runtime", "RocketMQ Broker 运行指标", "source /home/rocketmq/.bash_profile; mqadmin brokerStatus -n $NAMESRV_ADDR -b broker-a | egrep 'sendThreadPoolQueueSize|pullThreadPoolQueueSize|commitLogDiskRatio|consumeQueueDiskRatio'", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "Broker 线程池队列和存储使用率处于可接受范围 → OK；队列积压或磁盘水位过高 → WARN/CRIT"),
    _middleware_metric("local.rocketmq.jvm.gc", "RocketMQ JVM 参数与 GC 风险", "grep -R -iE 'Full GC|OutOfMemory|GC overhead' /opt/rocketmq/logs; grep -E 'Xms|Xmx|Xmn|UseG1GC|MaxGCPauseMillis' /opt/rocketmq/bin/runbroker.sh /opt/rocketmq/bin/runserver.sh", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "无 Full GC/OOM/GC overhead 关键记录且 JVM 参数合理 → OK；异常记录 → WARN/CRIT"),
    _middleware_metric("local.rocketmq.system.parameters", "RocketMQ 文件句柄与系统限制", "su - rocketmq -c 'ulimit -n; ulimit -u'; pid=$(pgrep -f 'NamesrvStartup|BrokerStartup' | head -1); cat /proc/$pid/limits | grep -E 'Max open files|Max processes'; sysctl vm.swappiness", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "nofile ≥65535 且进程限制满足部署基线 → OK；限制不足 → WARN/CRIT"),
    _middleware_metric("local.rocketmq.systemd.unit", "RocketMQ systemd 自启与服务定义", "systemctl is-enabled rocketmq-namesrv rocketmq-broker; systemctl cat rocketmq-namesrv rocketmq-broker", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "NameServer/Broker 服务定义存在、用户和 ExecStart 正确、启用自启 → OK；单元缺失或漂移 → CRIT"),
    _middleware_metric("local.rocketmq.log.data.retention", "RocketMQ 日志与存储文件增长", "du -sh /opt/rocketmq/logs /opt/rocketmq/store; find /opt/rocketmq/logs -type f -mtime +30 -ls; find /opt/rocketmq/store -type f -size +10G -ls", "Rocketmq运维巡检手册v1.0.docx", "P1-TABLE6", "历史日志和异常大文件受控，日志/存储目录可读 → OK；历史文件或大文件异常增长 → WARN/CRIT"),
