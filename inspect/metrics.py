...
KEEPALIVED_METRICS = [
    ...
    },
    {
        ...
        "name": "VIP 访问",
        "command": "curl -sS -I --connect-timeout {timeout} --max-time {timeout} http://127.0.0.1:{keepalived_port}/",
        "timeout_sec": 10,
        "parser": "parse_keepalived_vip_access",
        "unit": "枚举（reachable/unreachable）+ HTTP 状态",
        "source_anchor": "P0 指标表「VIP访问」行（仅在目标主机本机执行 curl -sS -I http://127.0.0.1:端口/；VIP 地址只用于配置/绑定核对）；" + _KEEPALIVED_ANCHOR,
        ...
        "conflicts": [],
        "doc_baseline": "优先读取配置中的 virtual_ipaddress 确认 VIP 配置，再使用 inspect.conf keepalived_port；为避免跨主机访问，HTTP 只在目标主机本机请求 http://127.0.0.1:端口/；返回 200/3xx/401/403 等可解释状态 → OK；连接失败或 5xx → CRIT",
...
ELASTICSEARCH_METRICS = [
    ...
    _es_metric("local.elasticsearch.version", "Elasticsearch 版本",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/",
    ...
    _es_metric("local.elasticsearch.cluster.health", "集群健康",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cluster/health?pretty",
    ...
    _es_metric("local.elasticsearch.nodes.online", "集群在线节点数",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cat/nodes?v&h=name,ip,node.role,master,heap.percent,cpu,load_1m,disk.used_percent",
    ...
    _es_metric("local.elasticsearch.nodes.cpu", "Elasticsearch 节点 CPU",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cat/nodes?v&h=name,ip,cpu,load_1m,load_5m,load_15m",
    ...
    _es_metric("local.elasticsearch.nodes.memory", "Elasticsearch 节点内存",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cat/nodes?v&h=name,heap.percent,ram.percent",
    ...
    _es_metric("local.elasticsearch.nodes.disk", "Elasticsearch 节点磁盘",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cat/allocation?v",
    ...
    _es_metric("local.elasticsearch.disk.watermark", "磁盘水位线",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} 'https://127.0.0.1:{elasticsearch_http_port}/_cluster/settings?include_defaults=true&filter_path=**.watermark*'",
    ...
    _es_metric("local.elasticsearch.shards.unassigned", "未分配分片",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cat/shards?v&h=index,shard,prirep,state,node,unassigned.reason",
    ...
    _es_metric("local.elasticsearch.heap.gc", "Elasticsearch Heap/GC",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cat/nodes?v&h=name,heap.percent; tail -n 200 {elasticsearch_gc_log} | grep -Ei 'Pause|Full|OutOfMemory|heap'",
    ...
    _es_metric("local.elasticsearch.thread_pool.rejected", "线程池拒绝",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_cat/thread_pool/search,write?v&h=node_name,name,active,queue,rejected,completed",
    ...
    _es_metric("local.elasticsearch.cluster.settings", "集群动态设置",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} 'https://127.0.0.1:{elasticsearch_http_port}/_cluster/settings?flat_settings=true&pretty'",
    ...
    _es_metric("local.elasticsearch.indices.health", "索引健康与规模",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} 'https://127.0.0.1:{elasticsearch_http_port}/_cat/indices?v&h=health,index,pri,rep,docs.count,store.size&s=store.size:desc'",
    ...
    _es_metric("local.elasticsearch.security.accounts", "安全账号与权限",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_security/user?pretty; curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_security/role?pretty",
    ...
    _es_metric("local.elasticsearch.snapshot.repository", "快照仓库",
                "curl -sS --connect-timeout {timeout} --max-time {timeout} {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_snapshot/_all?pretty; curl -sS --connect-timeout {timeout} --max-time {timeout} -X POST {elasticsearch_auth} https://127.0.0.1:{elasticsearch_http_port}/_snapshot/{elasticsearch_snapshot_repo}/_verify?pretty",
