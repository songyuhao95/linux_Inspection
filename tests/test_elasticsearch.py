...
def test_elasticsearch_service_port_uses_discovered_expected_ports():
    ...
    assert parsed["expected_ports"] == [19200, 19300]
    assert parsed["ports"] == [19200, 19300]
    assert normalize.JUDGERS["local.elasticsearch.service.port"](
        parsed,
        config.build_resolved_thresholds()["local.elasticsearch.service.port"],
        {},
    )["status"] == "OK"
