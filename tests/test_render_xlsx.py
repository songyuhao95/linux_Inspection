...
class TestConstants:
    ...
    def test_sheet_names_match_rr_section3(self):
        assert rx.SHEET_NAMES == (
            "Overview", "Local", "nginx", "keepalived", "elasticsearch", "Errors-Evidence",
        )
...
class TestRenderWithStubXlsxwriter:
    ...
    def test_errors_evidence_rows(self, monkeypatch, tmp_path):
        ...
        rx.render_xlsx(rich_docs(), tmp_path / "r.xlsx")
        wb = stub.workbooks[-1]
        errors = wb.sheets[5]  # Overview(0) Local(1) nginx(2) keepalived(3) elasticsearch(4) Errors-Evidence(5)
